# Data-Science-Project-Flight-Price-Prediction
A machine learning project to predict US airline flight fares using Deep Learning (Regression). Includes data cleaning, feature engineering, K-Means Clustering for data segmentation, and model performance evaluation
...
| :--- | :--- | :--- |
| **Model Performance ($R^2$ Score)** | **\[הכנס את ערך ה-$R^2$ כאן]** | Demonstrates the model's strong explanatory power over price variance. |
| **Prediction Error (MAE)** | **\[הכנס את ערך ה-MAE כאן]** (in USD/NIS) | Indicates high forecast reliability (low average prediction error). |
| **Clustering Insight** | Flights in Cluster 1 were, on average, **\[הכנס את ההבדל הממוצע במחיר כאן]** more expensive than Cluster 0. | Highlights valuable data segmentation for deeper analysis. |
